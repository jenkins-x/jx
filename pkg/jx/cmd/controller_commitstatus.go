...
func (o *ControllerCommitStatusOptions) onCommitStatusObj(obj interface{}, jxClient jenkinsv1client.Interface, ns string) {
	...
	if !ok {
		log.Logger().Fatalf("commit status controller: unexpected type %v\n", obj)
	} else {
		err := o.onCommitStatus(check, jxClient, ns)
		if err != nil {
			log.Logger().Fatalf("commit status controller: %v\n", err)
		}
	}
}
...
func (o *ControllerCommitStatusOptions) onPodObj(obj interface{}, jxClient jenkinsv1client.Interface, kubeClient kubernetes.Interface, ns string) {
	...
	if !ok {
		log.Logger().Fatalf("pod watcher: unexpected type %v\n", obj)
	} else {
		err := o.onPod(check, jxClient, kubeClient, ns)
		if err != nil {
			log.Logger().Fatalf("pod watcher: %v\n", err)
		}
	}
}
		...
		if labels != nil {
			...
			if buildName != "" {
				...
				if err != nil {
					...
					if o.Verbose {
						log.Logger().Infof("pod watcher: Unable to find PipelineActivity for %s\n", pipelineActName)
					}
				} else {
					...
					if o.Verbose {
						log.Logger().Infof("pod watcher: Adding lastCommitSha: %s and gitUrl: %s to %s\n", act.Spec.LastCommitSHA, act.Spec.GitURL, pipelineActName)
					}
				...
				if org != "" && repo != "" && buildNumber != "" && (pullBaseSha != "" || pullPullSha != "") {
					...
					if o.Verbose {
						log.Logger().Infof("pod watcher: build pod: %s, org: %s, repo: %s, buildNumber: %s, pullBaseSha: %s, pullPullSha: %s, pullRequest: %s, sourceUrl: %s\n", pod.Name, org, repo, buildNumber, pullBaseSha, pullPullSha, pullRequest, sourceUrl)
					}
					if sha == "" {
						log.Logger().Warnf("pod watcher: No sha on %s, not upserting commit status\n", pod.Name)
					} else {
						prow := prow.Options{
						...
						if o.Verbose {
							log.Logger().Infof("pod watcher: Using contexts %v\n", contexts)
						}
...
func (o *ControllerCommitStatusOptions) UpsertCommitStatusCheck(name string, pipelineActName string, url string, sha string, pullRequest string, context string, phase corev1.PodPhase, jxClient jenkinsv1client.Interface, ns string) error {
	if name != "" {
		...
		if err != nil {
			...
			log.Logger().Infof("pod watcher: commit status already exists for %s\n", name)
		}
		...
		if o.Verbose {
			log.Logger().Infof("pod watcher: Discovered possible status details %v\n", possibleStatusDetails)
		}
		if len(possibleStatusDetails) == 1 {
			if o.Verbose {
				log.Logger().Infof("CommitStatus %s for pipeline %s already exists\n", name, pipelineActName)
			}
		...
		if create {
			log.Logger().Infof("pod watcher: Creating commit status for pipeline activity %s\n", pipelineActName)
			...
			log.Logger().Infof("pod watcher: Adding commit status for pipeline activity %s\n", pipelineActName)
			...
			if o.Verbose {
				log.Logger().Infof("pod watcher: Not updating or creating pipeline activity %s\n", pipelineActName)
			}
		}
	} else {
