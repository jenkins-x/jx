			...
			case mode.IsDir():
				...
				if !o.NoGit {
					...
					if repo != nil && err == nil {
						...
						gitUrl, err = o.Git().GitCreatePushURL(repo.CloneURL, &userAuth)
						if err != nil {
							return err
						}
						log.Infof("Cloning %s into directory %s\n", util.ColorInfo(repo.CloneURL), util.ColorInfo(outPath))
						err = o.Git().GitCloneOrPull(gitUrl, outPath)
						if err != nil {
							return err
						}
					}
				...
				if !o.NoGit {
					if createRepo {
						...
						err := o.Git().GitInit(outPath)
						if err != nil {
							return err
						}
						err = o.Git().GitAddRemote(outPath, gitUrl, "origin")
						if err != nil {
							return err
						}
					}
					// ignore errors as probably already added
					o.Git().GitAdd(outPath, ".gitignore")
					o.Git().GitAdd(outPath, "src", "charts", "*")

					message := "generated by: jx step split monorepo"

					err = o.Git().GitCommitIfChanges(outPath, message)
					if err != nil {
						return err
					}
					err = o.Git().GitPushMaster(outPath)
					if err != nil {
						return err
					}
