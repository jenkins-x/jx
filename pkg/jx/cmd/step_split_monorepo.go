			...
			case mode.IsDir():
				...
				if !o.NoGit {
					...
					if repo != nil && err == nil {
						...
						gitUrl, err = gits.GitCreatePushURL(repo.CloneURL, &userAuth)
						if err != nil {
							return err
						}
						o.Printf("Cloning %s into directory %s\n", util.ColorInfo(repo.CloneURL), util.ColorInfo(outPath))
						err = gits.GitCloneOrPull(gitUrl, outPath)
						if err != nil {
							return err
						}
					}
				...
				if !o.NoGit {
					if createRepo {
						...
						o.Printf("Created git repository to %s\n\n", util.ColorInfo(repo.HTMLURL))

						userAuth := gitProvider.UserAuth()
						gitUrl, err = gits.GitCreatePushURL(repo.CloneURL, &userAuth)

						err := gits.GitInit(outPath)
						if err != nil {
							return err
						}
					...
					gits.GitAdd(outPath, ".gitignore")
					gits.GitAdd(outPath, "src", "charts", "*")

					message := "generated by: jx step split monorepo"

					err = gits.GitCommitIfChanges(outPath, message)
					if err != nil {
						return err
					}
...
func (o *CommonOptions) createGitProviderForURL(gitKind string, gitUrl string) (gits.GitProvider, error) {
	...
	return gits.CreateProviderForURL(authConfigSvc, gitKind, gitUrl)
}
