...
// CreateAppTestOptionsForApp configures the mock environment for running apps related tests for a given App
func CreateAppTestOptionsForApp(gitOps bool, appName string, t *testing.T) *AppTestOptions {
	mockFactory := cmd_test.NewMockFactory()
	commonOpts := opts.NewCommonOptionsWithFactory(mockFactory)
	o := AppTestOptions{
		CommonOptions: &commonOpts,
