...
func (o *UpgradePlatformOptions) Run() error {
	...
	if "" == settings.KubeProvider {
		logrus.Warnf("Unable to determine provider from team settings")
	...
	logrus.Infof("Using provider '%s' from team settings\n", util.ColorInfo(settings.KubeProvider))
	...
	if targetVersion != currentVersion {
		logrus.Infof("Upgrading platform from version %s to version %s\n", util.ColorInfo(currentVersion), util.ColorInfo(targetVersion))
	} else if o.AlwaysUpgrade {
		logrus.Infof("Rerunning platform version %s\n", util.ColorInfo(targetVersion))
	} else {
		logrus.Infof("Already installed platform version %s. Skipping upgrade process.\n", util.ColorInfo(targetVersion))
		return nil
	}
	...
	if invalidFormat {
		logrus.Warnf("We have detected that the %s file has an invalid format", adminSecretsFileName)
		...
		if confirm {
			...
			logrus.Error("Aborting upgrade due to invalid adminSecrets.yaml")
			return nil
		}
	}
