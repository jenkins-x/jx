...
func NewCmdCreateClusterMinikube(f clients.Factory, in terminal.FileReader, out terminal.FileWriter, errOut io.Writer) *cobra.Command {
	options := CreateClusterMinikubeOptions{
		CreateClusterOptions: createCreateClusterOptions(f, in, out, errOut, commoncmd.MINIKUBE),
	}
	cmd := &cobra.Command{
		Use:     "minikube",
...
func (o *CreateClusterMinikubeOptions) Run() error {
	var deps []string
	d := commoncmd.BinaryShouldBeInstalled("minikube")
	if d != "" {
		deps = append(deps, d)
	}
...
func (o *CreateClusterMinikubeOptions) createClusterMinikube() error {
	...
	err = o.initAndInstall(commoncmd.MINIKUBE)
	if err != nil {
		return err
	}
