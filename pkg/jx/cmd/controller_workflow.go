...
func (o *ControllerWorkflowOptions) Run() error {
	...
	logrus.Infof("Watching for PipelineActivity resources in namespace %s\n", util.ColorInfo(ns))
	...
	go func() {
		for t := range ticker.C {
			if o.Verbose {
				logrus.Infof("Polling to see if any PRs have merged: %v\n", t)
			}
...
func (o *ControllerWorkflowOptions) onWorkflowObj(obj interface{}, jxClient versioned.Interface, ns string) {
	...
	if !ok {
		logrus.Warnf("Object is not a Workflow %#v\n", obj)
		return
	}
	if workflow != nil {
		o.onWorkflow(workflow, jxClient, ns)
	...
	if pipeline != nil {
		...
		if err == nil {
			if kube.IsResourceVersionNewer(activity.ResourceVersion, pipeline.ResourceVersion) {
				if o.Verbose {
					logrus.Infof("onActivity %s using newer resourceVersion of PipelineActivity %s > %s\n", pipeline.Name, activity.ResourceVersion, pipeline.ResourceVersion)
				}
				pipeline = activity
			}
		}
...
func (o *ControllerWorkflowOptions) onActivity(pipeline *v1.PipelineActivity, jxClient versioned.Interface, ns string) {
	...
	if o.Verbose {
		logrus.Infof("Processing pipeline %s repo %s version %s with workflow %s and status %s\n", pipeline.Name, repoName, version, workflowName, string(pipeline.Spec.WorkflowStatus))
	}
	...
	if repoName == "" || version == "" || build == "" || pipelineName == "" {
		if o.Verbose {
			logrus.Infof("Ignoring missing data for pipeline: %s repo: %s version: %s status: %s\n", pipeline.Name, repoName, version, string(pipeline.Spec.WorkflowStatus))
		}
	...
	if !pipeline.Spec.WorkflowStatus.IsTerminated() {
		...
		if flow == nil && workflowName == "default" {
			...
			if err != nil {
				logrus.Warnf("Cannot create default Workflow: %s\n", err)
				o.removePipelineActivity(pipeline, activities)
				return
			}
		}
		...
		if !o.isReleaseBranch(branch) {
			logrus.Infof("Ignoring branch %s\n", branch)
			o.removePipelineActivity(pipeline, activities)
			return
		}
				...
				if envName != "" {
					...
					if status == nil || status.PullRequest == nil || status.PullRequest.PullRequestURL == "" {
						...
						if canExecuteStep(flow, pipeline, &step, promoteStatusMap, envName) {
							logrus.Infof("Creating PR for environment %s from PipelineActivity %s as current status is %#v\n", envName, pipeline.Name, status)
							po := o.createPromoteOptions(repoName, envName, pipelineName, build, version)

							err := po.Run()
							if err != nil {
								logrus.Warnf("Failed to create PullRequest on pipeline %s repo %s version %s with workflow %s: %s\n", pipeline.Name, repoName, version, workflowName, err)
							}
						}
					}
		...
		if allStepsComplete && (pipeline.Spec.Status != v1.ActivityStatusTypeSucceeded || pipeline.Spec.WorkflowStatus != v1.ActivityStatusTypeSucceeded) {
			...
			if err != nil {
				logrus.Warnf("Failed to update PipelineActivity %s due to being complete: %s", pipeline.Name, err)
			}
...
func (o *ControllerWorkflowOptions) ReloadAndPollGitPipelineStatuses(jxClient versioned.Interface, ns string) {
	...
	if err != nil {
		logrus.Warnf("failed to list PipelineActivity resources: %s", err)
	} else {
		for _, pipeline := range pipelines.Items {
			if o.Verbose {
				logrus.Infof("Polling git status of activity %s\n", pipeline.Name)
			}
...
func (o *ControllerWorkflowOptions) pollGitStatusforPipeline(activity *v1.PipelineActivity, activities typev1.PipelineActivityInterface, environments typev1.EnvironmentInterface, ns string) {
	...
	for _, step := range activity.Spec.Steps {
		...
		if promote.Status.IsTerminated() {
			if o.Verbose {
				logrus.Infof("Pipeline %s promote Environment %s ignored as status %s\n", activity.Name, promote.Environment, string(promote.Status))
			}
			continue
		}
		envName := promote.Environment
		...
		if pullRequestStep == nil {
			logrus.Infof("Pipeline %s promote Environment %s status %s ignored as no PullRequest\n", activity.Name, promote.Environment, string(promote.Status))
			continue
		}
		prURL := pullRequestStep.PullRequestURL
		if prURL == "" || envName == "" {
			logrus.Infof("Pipeline %s promote Environment %s status %s ignored for PR %s\n", activity.Name, promote.Environment, string(promote.Status), prURL)
			continue
		}
		gitProvider, gitInfo, err := o.createGitProviderForPR(prURL)
		if err != nil {
			logrus.Warnf("Failed to create git Provider: %s", err)
			return
		}
		...
		if err != nil {
			logrus.Warnf("Failed to get PR number: %s", err)
			return
		}
		pr, err := gitProvider.GetPullRequest(gitInfo.Organisation, gitInfo, prNumber)
		if err != nil {
			logrus.Warnf("Failed to query the Pull Request status on pipeline %s for repo %s PR %d for PR %s: %s", activity.Name, gitInfo.HttpsURL(), prNumber, prURL, err)
		} else {
			if o.Verbose {
				logrus.Infof("Pipeline %s promote Environment %s has PR %s\n", activity.Name, envName, prURL)
			}
			...
			if pr.Merged != nil && *pr.Merged {
				if pr.MergeCommitSHA == nil {
					logrus.Warnf("Pipeline %s promote Environment %s has PR %s which is merged but there is no merge SHA\n", activity.Name, envName, prURL)
				} else {
					...
					if err != nil {
						logrus.Warnf("Failed to find environment %s: %s\n", envName, err)
						return
					} else {
						...
						if err != nil {
							logrus.Warnf("Failed to get the jx client: %s\n", err)
							return
						}
						...
						if o.NoWaitForUpdatePipeline {
							logrus.Infof("Pull Request %d merged but we are not waiting for the update pipeline to complete!\n",
								prNumber)
							err = po.commentOnIssues(ns, env, promoteKey)
							if err != nil {
								logrus.Warnf("Failed to comment on issues: %s", err)
							}
							err = promoteKey.OnPromoteUpdate(jxClient, o.Namespace, kube.CompletePromotionUpdate)
							if err != nil {
								logrus.Warnf("PipelineActivity update failed while completing promotion step. activity=%s\n",
									activity.Name)
							}
							return
						}
						...
						if err == nil {
							...
							if len(statuses) > 0 {
								...
								if succeeded {
									gitURL := activity.Spec.GitURL
									if gitURL == "" {
										logrus.Warnf("No git URL for PipelineActivity %s so cannot comment on issues\n", activity.Name)
										return
									}
									gitInfo, err := gits.ParseGitURL(gitURL)
									if err != nil {
										logrus.Warnf("Failed to parse Git URL %s for PipelineActivity %s so cannot comment on issues: %s", gitURL, activity.Name, err)
										return
									}
									po.GitInfo = gitInfo
									err = po.commentOnIssues(ns, env, promoteKey)
									...
									err = promoteKey.OnPromoteUpdate(jxClient, o.Namespace, kube.CompletePromotionUpdate)
									if err != nil {
										logrus.Warnf("Failed to update PipelineActivity on promotion completion: %s", err)
									}
									return
								}
							}
				...
				if err != nil {
					...
					logrus.Infof("Pipeline %s promote Environment %s has PR %s with status %s\n", activity.Name, envName, prURL, status)
					...
					if status == "success" {
						...
						logrus.Warnf("Pull request %s last commit has status %s for ref %s", pr.URL, status, pr.LastCommitSha)
						return
					}
				}
			}
...
func canExecuteStep(workflow *v1.Workflow, activity *v1.PipelineActivity, step *v1.WorkflowStep, statusMap map[string]*v1.PromoteActivityStep, promoteToEnv string) bool {
	for _, envName := range step.Preconditions.Environments {
		...
		if status == nil {
			logrus.Warnf("Cannot promote to Environment: %s as precondition Environment: %s as no status\n", promoteToEnv, envName)
			return false
		}
		if status.Status != v1.ActivityStatusTypeSucceeded {
			logrus.Warnf("Cannot promote to Environment: %s as precondition Environment: %s has status %s", promoteToEnv, envName, string(status.Status))
			return false
		}
	}
	return true
...
func modifyPipeline(activities typev1.PipelineActivityInterface, activity *v1.PipelineActivity, callback func(activity *v1.PipelineActivity) bool) error {
	...
	if callback(activity) {
		if !reflect.DeepEqual(activity, &old) {
			...
			if err != nil {
				logrus.Warnf("Failed to update PipelineActivity %s: %s\n", activity.Name, err)
				return err
			}
		}
	}
...
func (o *ControllerWorkflowOptions) isNewestPipeline(activity *v1.PipelineActivity, activities typev1.PipelineActivityInterface) bool {
	...
	for _, p := range deleteNames {
		if o.Verbose {
			logrus.Infof("Removing old Pipeline version %s\n", p.Name)
		}
