...
func (o *CreateVaultOptions) createVault(vaultOperatorClient versioned.Interface, vaultName string, kubeProvider string) error {
	...
	logrus.Infof("Current Cluster: %s\n", util.ColorInfo(clusterName))
	...
	logrus.Infof("Created service account %s for Vault authentication\n", util.ColorInfo(vaultAuthServiceAccount))
...
func (o *CreateVaultOptions) createVaultGKE(vaultOperatorClient versioned.Interface, vaultName string, kubeClient kubernetes.Interface, clusterName string, vaultAuthServiceAccount string) error {
	...
	logrus.Infof("Ensure KMS API is enabled\n")
	err = gke.EnableAPIs(o.GKEProjectID, "cloudkms")
	if err != nil {
		return errors.Wrap(err, "unable to enable 'cloudkms' API")
	}

	logrus.Infof("Creating GCP service account for Vault backend\n")
	...
	logrus.Infof("%s service account created\n", util.ColorInfo(gcpServiceAccountSecretName))

	logrus.Infof("Setting up GCP KMS configuration\n")
	...
	logrus.Infof("GCS bucket %s was created for Vault backend\n", util.ColorInfo(vaultBucket))

	logrus.Infof("Creating Vault...\n")
...
func (o *CreateVaultOptions) createVaultAWS(vaultOperatorClient versioned.Interface, vaultName string,
	...
	if o.DynamoDBRegion == "" || o.KMSRegion == "" || o.S3Region == "" {
		...
		logrus.Infof("Region not specified, defaulting to %s\n", util.ColorInfo(defaultRegion))
		if o.DynamoDBRegion == "" {
