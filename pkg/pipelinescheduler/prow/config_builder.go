...
func Build(schedulers []*pipelinescheduler.SchedulerLeaf) (*config.Config, *plugins.Configuration,
	...
	pluginsResult := plugins.Configuration{
		ExternalPlugins:      make(map[string][]plugins.ExternalPlugin),
		Approve:              make([]plugins.Approve, 0),
		Plugins:              make(map[string][]string),
		Lgtm:                 make([]plugins.Lgtm, 0),
		Triggers:             make([]plugins.Trigger, 0),
		Label:                &plugins.Label{},
		Size:                 &plugins.Size{},
		Welcome:              make([]plugins.Welcome, 0),
		Blockades:            make([]plugins.Blockade, 0),
		Golint:               &plugins.Golint{},
		RepoMilestone:        make(map[string]plugins.Milestone),
		RequireMatchingLabel: make([]plugins.RequireMatchingLabel, 0),
	}
