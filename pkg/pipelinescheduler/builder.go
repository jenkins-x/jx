	...
	for i := len(schedulers) - 1; i >= 0; i-- {
		...
		if answer == nil {
			...
			if answer.Triggers == nil {
				answer.Triggers = parent.Triggers
			} else if parent.Triggers != nil {
				applyToTriggers(parent.Triggers, answer.Triggers)
			}
...
type ReplaceableSliceOfTriggers struct {
	Items   []*Trigger
	Replace bool
}

func applyToTriggers(parent *ReplaceableSliceOfTriggers, child *ReplaceableSliceOfTriggers) {
	if !child.Replace {
		if child.Items == nil {
			child.Items = make([]*Trigger, 0)
		}
...
func applyToPostSubmits(parent *Postsubmits, child *Postsubmits) error {
	...
	for _, parent := range parent.Items {
		...
		for _, postsubmit := range child.Items {
			if postsubmit.Name == parent.Name {
