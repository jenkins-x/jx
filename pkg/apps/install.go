...
func (o *InstallOptions) AddApp(app string, version string, repository string, username string, password string,
	valuesFiles []string, setValues []string, alias string, helmUpdate bool) error {
	inspectChartFunc := o.createInspectChartFn(version, app, repository, username, password, setValues,
		alias, helmUpdate)
...
func (o *InstallOptions) createInspectChartFn(version string, app string, repository string, username string,
	password string, setValues []string, alias string, helmUpdate bool) func(dir string) error {
	var schema []byte
	var values []byte
	inspectChartFunc := func(dir string) error {
		...
		if o.GitOps {
			...
			err := opts.AddApp(app, dir, version, values, repository, username, password, setValues,
				helmUpdate)
			if err != nil {
				return err
			}
