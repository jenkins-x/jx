...
func EnsurePluginInstalled(plugin jenkinsv1.Plugin) (string, error) {
	...
	if _, err = os.Stat(path); os.IsNotExist(err) {
		...
		logrus.Infof("Installing plugin %s version %s for command %s from %s\n", util.ColorInfo(plugin.Spec.Name),
		...
		for _, f := range files {
			if strings.HasPrefix(f.Name(), plugin.Name) {
				...
				if err != nil {
					logrus.Warnf("Unable to delete old version of plugin %s installed at %s because %v\n", plugin.Name, f.Name(), err)
				} else {
		...
		if len(deleted) > 0 {
			logrus.Infof("Deleted old plugin versions: %v\n", util.ColorInfo(deleted))
		}
		...
		defer func() {
			...
			if err != nil {
				logrus.Errorf("Error cleaning up tmpdir %s because %v\n", tmpDir, err)
			}
		}()
		if err != nil {
			return "", err
...
func ValidatePlugins(jxClient jenkinsv1client.Interface, ns string) error {
	...
	for subCommand, ps := range seenSubCommands {
		if len(ps) > 1 {
			logrus.Warnf("More than one extension has installed a plugin which will be called for jx %s. These extensions are:\n", util.ColorWarning(subCommand))
			for _, p := range ps {
