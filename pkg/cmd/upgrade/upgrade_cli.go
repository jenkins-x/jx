...
import (
	...
	goupdate "github.com/minio/selfupdate"
...
func (*CLIOptions) InstallJx(upgrade bool, version string) error {
	...
	if err != nil {
		...
		log.Logger().Debugf("Signature verification successful for %s", termcolor.ColorInfo(clientURL))
	}
	...
	reader, err := jxutil.UncompressArtifact(artifactPath, clientURL, binary)
	if err != nil {
