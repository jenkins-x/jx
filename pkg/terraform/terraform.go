...
func Init(terraformDir string, serviceAccountPath string) error {
	logrus.Info("Initialising Terraform")

	if _, err := os.Stat(".terraform"); !os.IsNotExist(err) {
		logrus.Info("Discovered local .terraform directory, removing...")
...
func Plan(terraformDir string, terraformVars string, serviceAccountPath string) (string, error) {
	logrus.Info("Showing Terraform Plan")
	cmd := util.Command{
		Name: "terraform",
		Args: []string{"plan",
...
func Apply(terraformDir string, terraformVars string, serviceAccountPath string, stdout io.Writer, stderr io.Writer) error {
	logrus.Info("Applying Terraform")
	cmd := util.Command{
		Name: "terraform",
...
func CheckVersion() error {
	logrus.Info("Checking Terraform Version...")
	cmd := util.Command{
		Name: "terraform",
		Args: []string{"-version"},
	}
	...
	logrus.Infof("Determined terraform version as %s\n", util.ColorInfo(version))
	...
	logrus.Infof("Terraform version appears to be valid\n")
