// Code generated by pegomock. DO NOT EDIT.
// Source: github.com/jenkins-x/jx/v2/pkg/kube (interfaces: Kuber)

package kube_test

import (
	"reflect"
	"time"

	pegomock "github.com/petergtz/pegomock"
	clientcmd "k8s.io/client-go/tools/clientcmd"
	api "k8s.io/client-go/tools/clientcmd/api"
)

type MockKuber struct {
	fail func(message string, callerSkip ...int)
}

func NewMockKuber(options ...pegomock.Option) *MockKuber {
	mock := &MockKuber{}
	for _, option := range options {
		option.Apply(mock)
	}
	return mock
}

func (mock *MockKuber) SetFailHandler(fh pegomock.FailHandler) { mock.fail = fh }
func (mock *MockKuber) FailHandler() pegomock.FailHandler      { return mock.fail }

func (mock *MockKuber) LoadConfig() (*api.Config, *clientcmd.PathOptions, error) {
	if mock == nil {
		panic("mock must not be nil. Use myMock := NewMockKuber().")
	}
	params := []pegomock.Param{}
	result := pegomock.GetGenericMockFrom(mock).Invoke("LoadConfig", params, []reflect.Type{reflect.TypeOf((**api.Config)(nil)).Elem(), reflect.TypeOf((**clientcmd.PathOptions)(nil)).Elem(), reflect.TypeOf((*error)(nil)).Elem()})
	var ret0 *api.Config
	var ret1 *clientcmd.PathOptions
	var ret2 error
	if len(result) != 0 {
		if result[0] != nil {
			ret0 = result[0].(*api.Config)
		}
		if result[1] != nil {
			ret1 = result[1].(*clientcmd.PathOptions)
		}
		if result[2] != nil {
			ret2 = result[2].(error)
		}
	}
	return ret0, ret1, ret2
}

func (mock *MockKuber) UpdateConfig(_param0 string, _param1 string, _param2 string, _param3 string, _param4 string) error {
	if mock == nil {
		panic("mock must not be nil. Use myMock := NewMockKuber().")
	}
	params := []pegomock.Param{_param0, _param1, _param2, _param3, _param4}
	result := pegomock.GetGenericMockFrom(mock).Invoke("UpdateConfig", params, []reflect.Type{reflect.TypeOf((*error)(nil)).Elem()})
	var ret0 error
	if len(result) != 0 {
		if result[0] != nil {
			ret0 = result[0].(error)
		}
	}
	return ret0
}

func (mock *MockKuber) VerifyWasCalledOnce() *VerifierMockKuber {
	return &VerifierMockKuber{
		mock:                   mock,
		invocationCountMatcher: pegomock.Times(1),
	}
}

func (mock *MockKuber) VerifyWasCalled(invocationCountMatcher pegomock.Matcher) *VerifierMockKuber {
	return &VerifierMockKuber{
		mock:                   mock,
		invocationCountMatcher: invocationCountMatcher,
	}
}

func (mock *MockKuber) VerifyWasCalledInOrder(invocationCountMatcher pegomock.Matcher, inOrderContext *pegomock.InOrderContext) *VerifierMockKuber {
	return &VerifierMockKuber{
		mock:                   mock,
		invocationCountMatcher: invocationCountMatcher,
		inOrderContext:         inOrderContext,
	}
}

func (mock *MockKuber) VerifyWasCalledEventually(invocationCountMatcher pegomock.Matcher, timeout time.Duration) *VerifierMockKuber {
	return &VerifierMockKuber{
		mock:                   mock,
		invocationCountMatcher: invocationCountMatcher,
		timeout:                timeout,
	}
}

type VerifierMockKuber struct {
	mock                   *MockKuber
	invocationCountMatcher pegomock.Matcher
	inOrderContext         *pegomock.InOrderContext
	timeout                time.Duration
}

func (verifier *VerifierMockKuber) LoadConfig() *MockKuber_LoadConfig_OngoingVerification {
	params := []pegomock.Param{}
	methodInvocations := pegomock.GetGenericMockFrom(verifier.mock).Verify(verifier.inOrderContext, verifier.invocationCountMatcher, "LoadConfig", params, verifier.timeout)
	return &MockKuber_LoadConfig_OngoingVerification{mock: verifier.mock, methodInvocations: methodInvocations}
}

type MockKuber_LoadConfig_OngoingVerification struct {
	mock              *MockKuber
	methodInvocations []pegomock.MethodInvocation
}

func (c *MockKuber_LoadConfig_OngoingVerification) GetCapturedArguments() {
}

func (c *MockKuber_LoadConfig_OngoingVerification) GetAllCapturedArguments() {
}

func (verifier *VerifierMockKuber) UpdateConfig(_param0 string, _param1 string, _param2 string, _param3 string, _param4 string) *MockKuber_UpdateConfig_OngoingVerification {
	params := []pegomock.Param{_param0, _param1, _param2, _param3, _param4}
	methodInvocations := pegomock.GetGenericMockFrom(verifier.mock).Verify(verifier.inOrderContext, verifier.invocationCountMatcher, "UpdateConfig", params, verifier.timeout)
	return &MockKuber_UpdateConfig_OngoingVerification{mock: verifier.mock, methodInvocations: methodInvocations}
}

type MockKuber_UpdateConfig_OngoingVerification struct {
	mock              *MockKuber
	methodInvocations []pegomock.MethodInvocation
}

func (c *MockKuber_UpdateConfig_OngoingVerification) GetCapturedArguments() (string, string, string, string, string) {
	_param0, _param1, _param2, _param3, _param4 := c.GetAllCapturedArguments()
	return _param0[len(_param0)-1], _param1[len(_param1)-1], _param2[len(_param2)-1], _param3[len(_param3)-1], _param4[len(_param4)-1]
}

func (c *MockKuber_UpdateConfig_OngoingVerification) GetAllCapturedArguments() (_param0 []string, _param1 []string, _param2 []string, _param3 []string, _param4 []string) {
	params := pegomock.GetGenericMockFrom(c.mock).GetInvocationParams(c.methodInvocations)
	if len(params) > 0 {
		_param0 = make([]string, len(c.methodInvocations))
		for u, param := range params[0] {
			_param0[u] = param.(string)
		}
		_param1 = make([]string, len(c.methodInvocations))
		for u, param := range params[1] {
			_param1[u] = param.(string)
		}
		_param2 = make([]string, len(c.methodInvocations))
		for u, param := range params[2] {
			_param2[u] = param.(string)
		}
		_param3 = make([]string, len(c.methodInvocations))
		for u, param := range params[3] {
			_param3[u] = param.(string)
		}
		_param4 = make([]string, len(c.methodInvocations))
		for u, param := range params[4] {
			_param4[u] = param.(string)
		}
	}
	return
}
