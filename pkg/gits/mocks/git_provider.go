...
func (mock *MockGitProvider) Server() auth.Server {
	if mock == nil {
		panic("mock must not be nil. Use myMock := NewMockGitProvider().")
	}
	params := []pegomock.Param{}
	result := pegomock.GetGenericMockFrom(mock).Invoke("Server", params, []reflect.Type{reflect.TypeOf((*auth.Server)(nil)).Elem()})
	var ret0 auth.Server
	if len(result) != 0 {
		if result[0] != nil {
			ret0 = result[0].(auth.Server)
		}
	}
	return ret0
}
...
func (verifier *VerifierMockGitProvider) Server() *MockGitProvider_Server_OngoingVerification {
	params := []pegomock.Param{}
	...
	return &MockGitProvider_Server_OngoingVerification{mock: verifier.mock, methodInvocations: methodInvocations}
}

type MockGitProvider_Server_OngoingVerification struct {
	mock              *MockGitProvider
	methodInvocations []pegomock.MethodInvocation
}

func (c *MockGitProvider_Server_OngoingVerification) GetCapturedArguments() {
}

func (c *MockGitProvider_Server_OngoingVerification) GetAllCapturedArguments() {
}
