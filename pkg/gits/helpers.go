...
func PushRepoAndCreatePullRequest(dir string, gitInfo *GitRepository, base string, prDetails *PullRequestDetails, filter *PullRequestFilter, commit bool, commitMessage string, push bool, autoMerge bool, dryRun bool, gitter Gitter, provider GitProvider) (*PullRequestInfo, error) {
	...
	if filter != nil && push {
		...
		if len(existingPrs) > 1 {
			...
			if pr.HeadRef != nil && pr.Number != nil {
				...
				if dryRun {
					log.Logger().Infof("Commit created but not pushed; would have updated pull request %s. Please manually delete %s when you are done.", util.ColorInfo(pr.URL), util.ColorInfo(dir))
					return nil, nil
				}
	...
	if dryRun {
		log.Logger().Infof("Commit created but not pushed; would have created new pull request. Please manually delete %s when you are done.", util.ColorInfo(dir))
		return nil, nil
	}
