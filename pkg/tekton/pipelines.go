...
func StructureForPipelineRun(jxClient versioned.Interface, ns string, run *pipelineapi.PipelineRun) (*v1.PipelineStructure, error) {
	structure, err := jxClient.JenkinsV1().PipelineStructures(ns).Get(run.Name, metav1.GetOptions{})
	if err != nil {
		if apierrors.IsNotFound(err) {
			// Try again but with the Pipeline name for this run.
			...
			return nil, errors.Wrapf(err, "getting PipelineStructure with PipelineRun name for PipelineRun %s", run.Name)
		}
	}
