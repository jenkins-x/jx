...
func TestVerifySignature(t *testing.T) {
	...
	err = os.WriteFile(artifactPath, content, 0600)
	require.NoError(t, err)
	...
	err = os.WriteFile(signaturePath, []byte(base64.StdEncoding.EncodeToString(signature)), 0600)
	require.NoError(t, err)
	...
	err = os.WriteFile(artifactPath, []byte("corrupted"), 0600)
	require.NoError(t, err)
	err = VerifySignature(artifactPath, signaturePath, pubPEM)
	assert.Error(t, err)
	...
	err = os.WriteFile(artifactPath, content, 0600) // restore content
	require.NoError(t, err)
