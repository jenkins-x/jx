		...
		for i, k := range keys {
			...
			if i < len(keys)-1 {
				...
				if !ok1 {
					...
					if !ok2 {
						return nil, fmt.Errorf("%s is not an associative array", kPath)
					} else {
						x = v2
					}
				}
...
func recurse(element interface{}, files map[string]string, kPath string) error {
		...
		for k, v := range e {
			if path, ok := files[k]; ok {
				if v == nil || util.IsZeroOfUnderlyingType(v) {
					...
					return fmt.Errorf("value at %s must be empty but is %v", kPath, v)
				}
			} else {
				// keep on recursing
				kPath = fmt.Sprintf("%s.%s", kPath, k)
				err := recurse(v, files, kPath)
				if err != nil {
					return err
				}
			}
