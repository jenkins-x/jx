...
func (h *HelmTemplate) kubectlApply(ns string, chart string, releaseName string, wait bool, create bool, dir string) error {
	logrus.Infof("Applying generated chart %s YAML via kubectl in dir: %s\n", chart, dir)
...
func (h *HelmTemplate) kubectlApplyFile(ns string, helmHook string, wait bool, create bool, file string) error {
	logrus.Infof("Applying Helm hook %s YAML via kubectl in file: %s\n", helmHook, file)
...
func (h *HelmTemplate) kubectlDeleteFile(ns string, file string) error {
	logrus.Infof("Deleting helm hook sources from file: %s\n", file)
...
func (h *HelmTemplate) deleteResourcesAndClusterResourcesBySelector(ns string, selector string, wait bool, message string) error {
	...
	logrus.Infof("Removing Kubernetes resources from %s using selector: %s from %s\n", message, util.ColorInfo(selector), strings.Join(kinds, " "))
...
func (h *HelmTemplate) fetchChart(chart string, version string, dir string, repo string, username string,
	...
	logrus.Infof("Fetched chart %s to dir %s\n", chart, answer)
	return answer, nil
}
		...
		if ext == ".yaml" {
			...
			for _, file := range objFiles {
				...
				if helmHook != "" {
					...
					if err != nil {
						logrus.Warnf("Failed to move helm hook template %s to %s: %s", path, newPath, err)
						return err
					}
...
func (h *HelmTemplate) deleteHooks(hooks []*HelmHook, hookPhase string, hookDeletePolicy string, ns string) error {
	...
	for _, hook := range matchingHooks {
		...
		name := hook.Name
		if kind == "Job" && name != "" {
			logrus.Infof("Waiting for helm %s hook Job %s to complete before removing it\n", hookPhase, name)
			err := kube.WaitForJobToComplete(h.KubeClient, ns, name, time.Minute*30, false)
			if err != nil {
				logrus.Warnf("Job %s has not yet terminated for helm hook phase %s due to: %s so removing it anyway\n", name, hookPhase, err)
			}
		} else {
			logrus.Warnf("Could not wait for hook resource to complete as it is kind %s and name %s for phase %s\n", kind, name, hookPhase)
		}
		if flag == "true" {
			logrus.Infof("Not deleting the Job %s as we have the $JX_DISABLE_DELETE_HELM_HOOKS enabled\n", name)
			continue
		}
