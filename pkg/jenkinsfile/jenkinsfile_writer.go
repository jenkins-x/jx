...
func (s *JenkinsfileStatement) ContextEquals(that *JenkinsfileStatement) bool {
	if s.Function == that.Function && contextFunctions[s.Function] {
		return reflect.DeepEqual(s.Arguments, that.Arguments)
	}
	return false
}
...
func (w *JenkinsfileWriter) Write(statements []JenkinsfileStatement) {
	statements = w.combineSimilarContexts(statements)
...
func (w *JenkinsfileWriter) combineSimilarContexts(statements []JenkinsfileStatement) []JenkinsfileStatement {
	...
	for i, _ := range answer {
		answer[i].Children = w.combineSimilarContexts(answer[i].Children)
	}
	return answer
}
