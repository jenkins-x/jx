	...
	return func(requirements *helm.Requirements, chart *helmchart.Metadata, values map[string]interface{},
		...
		for _, d := range requirements.Dependencies {
			if d.Name == chartName && d.Alias == alias {
				// App found
				log.Logger().Infof("App %s already installed.\n", util.ColorWarning(chartName))
...
func CreateNestedRequirementDir(dir string, requirementName string, requirementDir string, requirementVersion string,
	...
	if verbose {
		log.Logger().Infof("Using %s for requirementName files\n", appDir)
	}
	if requirementValuesFiles != nil && len(requirementValuesFiles.Items) > 0 {
		...
		if verbose {
			log.Logger().Infof("Writing values file to %s\n", rootValuesFileName)
		}
	}
	...
	if _, err := os.Stat(templatesDir); os.IsNotExist(err) {
		if verbose {
			log.Logger().Infof("No templates directory exists in %s\n", util.ColorInfo(dir))
		}
	} else if err != nil {
	...
	if _, err := os.Stat(chartYamlPath); err == nil {
		...
		if verbose {
			log.Logger().Infof("Not adding %s as not present in chart. Only files in %s are:\n", helm.ChartFileName,
				requirementDir)
	...
	if verbose && appReadme == "" {
		log.Logger().Infof("Not adding App Readme as no README, README.md, readme or readme.md found in %s\n", requirementDir)
	}
	...
	if err != nil {
		...
		if verbose {
			log.Logger().Infof("Writing README.md to %s\n", readmeOutPath)
		}
		externalFileHandler := func(path string, element map[string]interface{}, key string) error {
			...
			if schemaPaths, ok := schemas[schemaKey]; ok {
				for _, schemaPath := range schemaPaths {
					...
					if verbose {
						log.Logger().Infof("Writing %s to %s\n", fileName, schemaOutPath)
					}
				}
			}
			return nil
