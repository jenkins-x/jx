...
func CreateIssueProvider(kind string, server auth.Server, project string, batchMode bool, git gits.Gitter) (IssueProvider, error) {
	switch kind {
	case Jira:
		return CreateJiraIssueProvider(server, project, batchMode, git)
	default:
