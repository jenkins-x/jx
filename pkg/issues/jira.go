...
func CreateJiraIssueProvider(server *auth.AuthServer, userAuth *auth.UserAuth, project string, batchMode bool, git gits.Gitter) (IssueProvider, error) {
	...
	if userAuth != nil && !userAuth.IsInvalid() {
		...
		if batchMode {
			logrus.Infof("Using JIRA server %s user name %s and API token %s\n", u, username, strings.Repeat("*", len(userAuth.ApiToken)))
		}
	} else {
		if batchMode {
			if userAuth != nil && userAuth.Username != "" {
				logrus.Warnf("No API token found for JIRA server %s user %s so using anonymous access\n", u, userAuth.Username)
			} else {
				logrus.Warnf("No authentication found for JIRA server %s so using anonymous access\n", u)
			}
		}
	}
...
func (i *JiraService) SearchIssuesClosedSince(t time.Time) ([]*gits.GitIssue, error) {
	logrus.Warn("TODO SearchIssuesClosedSince() not yet implemented for JIRA")
	return nil, nil
}
