## jx upgrade

Upgrades resources

### Usage

```
jx upgrade
```

### Synopsis

Upgrades all of the plugins in your local JayeX CLI

### Examples

  # upgrades your plugin binaries
  jx upgrade

### Options

```
  -h, --help   help for upgrade
```

### SEE ALSO

* [jx](jx.md)	 - JayeX 3.x command line
* [jx upgrade cli](jx_upgrade_cli.md)	 - Upgrades your local JayeX CLI
* [jx upgrade plugins](jx_upgrade_plugins.md)	 - Upgrades all of the plugins in your local JayeX CLI

###### Auto generated by spf13/cobra on 16-Jun-2026
