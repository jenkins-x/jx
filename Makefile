...
docker-test:
	docker run --rm -v $(shell pwd):/jx golang:1.11 sh -c "cd /jx && make test"

docker-test-slow:
	docker run --rm -v $(shell pwd):/jx golang:1.11 sh -c "cd /jx && make test-slow"

# EASY WAY TO TEST IF YOUR TEST SHOULD BE A UNIT OR INTEGRATION TEST
docker-test-integration:
	docker run --rm -v $(shell pwd):/jx golang:1.11 sh -c "/jx && make test-integration"

# EASY WAY TO TEST IF YOUR SLOW TEST SHOULD BE A UNIT OR INTEGRATION TEST
docker-test-slow-integration:
	docker run --rm -v $(shell pwd):/jx golang:1.11 sh -c "cd /jx && make test-slow-integration"
